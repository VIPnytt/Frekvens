...
Frekvens is an ESP32-based mod designed specifically for [IKEA Frekvens](wiki/Frekvens) and [IKEA Obegränsad](wiki/Obegransad) devices.
...
Featuring replicas of all the same [modes](wiki/Modes), but incluses everything from new animations and weather services to user-interactive activities and smart-home [extensions](wiki/Extensions).

The buttons can do more and the [mic](wiki/Microphone) still syncs with the music. New capabilities have also been added, like [IR](wiki/Infrared) remote-control support, a feature [IKEA Frekvens](wiki/Frekvens) was designed for, but never shipped with.
...
For newcomers, it can be a bit challanging to derermine which wire needs to be connected to what pin, but since there's litterally hundreds of ESP32 different boards on the market, there won't be a single set to rule them all. Therefore the documentation will always points out what kind of pin should be used, so that it's easy to look up compatible pins on any board. If in doubt, just [ask](discussions).

### ➕ Accessories

If desired, extra hardware can be added:

- [IR](wiki/Infrared)-receiver
- [Microphone](wiki/Microphone)
- [RTC](wiki/RTC)-module

**Note:** *[IKEA Frekvens](wiki/Frekvens) already has a built-in microphone.*

### 🧵 Wiring

Both [IKEA Frekvens](wiki/Frekvens) and [IKEA Obegränsad](wiki/Obegransad) have dedicated setup guides explaining everything in detail, but essentially everything you'll have to do is wiring up power, ground, SPI, button and optional accessories to the designated pads.

- [IKEA Frekvens](wiki/Frekvens#-getting-started)
  - Remove the `U2` chip.
  - The microphone can be wired in from either `U3` pin 7 or `U2` pin 11.
- [IKEA Obegränsad](wiki/Obegransad#getting-started)
  - Remove the `U1` chip.
...
For more info, check out the [Time zone](wiki/Services#-time-zone) section in the Wiki.
...
For more info, check out the [Wi-Fi](wiki/Services#-wi-fi) section in the Wiki.
...
For more info, check out the [Weather](wiki/Services#-weather) section in the Wiki.
...
For more info, check out the [Display](wiki/Services#-display) section in the Wiki.
...
Make sure to set the correct *host*, or *IP address* in the `upload_port` option. If [password protection](wiki/Extensions#-ota) has been enabled using `OTA_KEY` or `OTA_KEY_HASH`, make sure also the password is specified in the `--auth` option of `upload_flags`.
...
For more info, check out the [OTA](wiki/Extensions#-ota) section in the Wiki.
...
In theory, any custom smart-home system can be integrated using [RESTful](wiki/Extensions#-restful), [MQTT](wiki/Extensions#-mqtt) or [WebSocket](wiki/Extensions#-websocket). However, when it comes to ready-made integrations that are easy to set up for end-users, these are currently two options:
...
For more info, check out the [Alexa](wiki/Extensions#-alexa) section in the Wiki.
...
For more info, check out the [Home Assistant](wiki/Extensions#-home-assistant) section in the Wiki.
...
- [RESTful](wiki/Extensions#-restful)
- [MQTT](wiki/Extensions#-mqtt)
- [WebSocket](wiki/Extensions#-websocket)
...
Please visit the [Wiki](wiki) to learn more info about what's supported in each [service](wiki/Services), [extension](wiki/Extensions) or [mode](wiki/Modes).
...
If you're seeing *"snow"*, this is usually an indication of EMI caused by bad wiring and/or connections. Keep it short and avoid pluggable connectors, especially jumper-wires and breadboards. Noise from some power supplies can also cause trouble, which in most cases can be prevented by installing capacitors. Make sure to follow the guidelines for your specific device, [IKEA Frekvens](wiki/Frekvens) or [IKEA Obegränsad](wiki/Obegransad).

If the display otherwise is looking normal, try increasing the [frame rate](wiki/Services#-display).

### The device reboots randomly

The [IKEA Obegränsad](wiki/Obegransad) draws approximately 2.0 A @ 5 V when idling and fully illuminated at maximum brightness. Ensure that the USB power supply can deliver at least 10 W of continuous power.
