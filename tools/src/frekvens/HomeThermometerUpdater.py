...
class HomeThermometerUpdater:
    ...
    def set(self, indoor: float | None = None, outdoor: float | None = None) -> httpx.Response:
        return httpx.patch(
    ...
    def setMode(self) -> httpx.Response:
        return httpx.patch(f"http://{self.host}/restful/Modes", json={"mode": self.mode}).raise_for_status()
...
def main() -> None:
    ...
    thermometer = HomeThermometerUpdater(args.host) if args.host else HomeThermometerUpdater()
