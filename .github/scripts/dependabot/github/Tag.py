...
class Git(GitHub.Client):
    def __init__(self, ctx: GitHub.Context) -> None:
        super().__init__(ctx)
        self.regex = re.compile(
            r"(?:git|git\+https|git\+ssh|https)://github\.com/(?P<owner>[^/;\s]+)/(?P<repo>[^/;\s]+)\.git#(?P<tag>[^/;\s]+)(?:\s*;\s*[0-9a-f]{7,40})?$",
            re.MULTILINE,
        )
            ...
            GitHub.ResultFragment(
                {
                    ...
                    "version_new": fragment["tag_new"].removeprefix("v"),
                    "string_old": fragment["string_old"],
                    "string_new": f"git+https://github.com/{fragment['owner']}/{fragment['repo']}#{fragment['tag_new']} ; {fragment['commit_new'][:7]}",
                }
            )
            for fragment in fragments
        ]
