...
void ConnectivityService::initStation()
{
    ...
    {
        const char *ssid_ptr = reinterpret_cast<const char *>(config.sta.ssid); // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast)
        const std::string_view ssid(ssid_ptr, strnlen(ssid_ptr, sizeof(config.sta.ssid)));
        if (!ssid.empty())
        {
            const char *key_ptr = reinterpret_cast<const char *>(config.sta.password); // NOLINT(cppcoreguidelines-pro-type-reinterpret-cast)
