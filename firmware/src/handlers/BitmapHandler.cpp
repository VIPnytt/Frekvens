...
void BitmapHandler::draw(uint8_t brightness) { draw((GRID_COLUMNS - width) / 2, (GRID_ROWS - height) / 2, brightness); }
...
uint8_t BitmapHandler::getHeight() const { return height; }

uint8_t BitmapHandler::getWidth() const { return width; }
